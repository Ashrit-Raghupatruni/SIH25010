  ...
  return (
    <>
      ...
      <button 
        ...
        className={`${isOpen ? 'hidden' : 'flex'} fixed bottom-6 right-6 bg-krishi-green text-white p-4 rounded-full shadow-xl hover:bg-krishi-darkGreen transition-all z-50 animate-bounce items-center justify-center`}
      >
        <Bot size={28} />
      </button>
      ...
      <div className={`${isOpen ? 'flex' : 'hidden'} fixed bottom-6 right-6 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl flex-col overflow-hidden z-50 border border-gray-100 h-[500px] transition-all duration-300 ease-in-out`}>
        {/* Header */}
        <div className="bg-krishi-green text-white p-4 flex justify-between items-center">
          <div className="flex items-center space-x-2">
            <Bot size={24} />
            <h3 className="font-semibold text-lg">AgriBot Assistant</h3>
          </div>
          <div className="flex items-center space-x-3">
             <button onClick={toggleSpeaker} className="text-white/80 hover:text-white transition-colors" title="Toggle Auto-Voice">
        ...
        <div className="flex-1 p-4 overflow-y-auto bg-gray-50 flex flex-col space-y-4">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] p-3 rounded-2xl text-sm shadow-sm ${msg.sender === 'user' ? 'bg-krishi-green text-white rounded-tr-none' : 'bg-white text-gray-800 border border-gray-100 rounded-tl-none'}`}>
                {msg.text}
              </div>
            </div>
          ))}
        ...
        <div className="p-4 bg-white border-t border-gray-100">
          <div className="flex items-center bg-gray-50 rounded-xl p-1 border border-gray-200">
            ...
            <button 
              ...
              className="p-2 bg-krishi-green text-white rounded-lg hover:bg-krishi-darkGreen transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Send size={18} />
            </button>
          </div>
