...
const Weather = () => {
  ...
  const fetchWeather = async (loc?: string) => {
    ...
    try {
      ...
      setData({ temperature: 28, humidity: 72, rainChance: 35, description: "Partly Cloudy" });
    } catch {
  ...
  const stats = data
    ? [
        { icon: Thermometer, label: t("temperature"), value: `${data.temperature}°C`, color: "text-secondary" },
        { icon: Droplets, label: t("humidity"), value: `${data.humidity}%`, color: "text-sky" },
        { icon: CloudRain, label: t("rain_chance"), value: `${data.rainChance}%`, color: "text-primary" },
      ]
    : [];
    ...
    <div className="mx-auto max-w-lg space-y-6">
      ...
      <div className="space-y-3 rounded-2xl border bg-card p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button onClick={() => fetchWeather()} disabled={loading || !location} size="lg" className="rounded-xl bg-sky py-5 text-lg font-bold text-sky-foreground">
        ...
        {data && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              {stats.map((s) => (
                <div key={s.label} className="flex flex-col items-center gap-2 rounded-2xl border bg-card p-5 shadow-sm">
